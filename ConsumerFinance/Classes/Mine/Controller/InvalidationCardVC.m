...
#import "InvalidationCardVC.h"
#import "OccupiedCardCell.h"
#import "OccupiedCardDetailVC.h"

@interface InvalidationCardVC ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation InvalidationCardVC
...
- (void)viewDidLoad {
    ...
    self.page = 1;
    [self loadMeteringCardInvalidationList];
}
...
- (void)loadMeteringCardInvalidationList {
    [MBProgressHUD showMessage:nil toView:self.view];  //计次卡：S_CKZT_SYZ-使用中，S_CKZT_YSX-已失效
    NSDictionary *parameters = @{@"type":@"S_CKZT_YSX", @"page":[NSString stringWithFormat:@"%d",self.page], @"pageSize":@10};
