...
- (void)show {
    ...
    self.month = [NSString stringWithFormat:@"%ld月", (long)[self.timeArr[1] integerValue]];
    self.day = [NSString stringWithFormat:@"%ld日", (long)[self.timeArr[2] integerValue]];
...
//    [self.pickerView selectRow:self.minuteInterval == 1 ? ([self.minuteArr indexOfObject:self.minute]) : (self.minuteArr.count / 2) inComponent:4 animated:YES];
    ...
    int index = [self.timeArr[4] intValue] / self.minuteInterval;
    [self.pickerView selectRow:[self.yearArr indexOfObject:self.year] inComponent:0 animated:YES];
    /// 重新格式化转一下，是因为如果是09月/日/时，数据源是9月/日/时,就会出现崩溃
    [self.pickerView selectRow:[self.monthArr indexOfObject:self.month] inComponent:1 animated:YES];
    [self.pickerView selectRow:[self.dayArr indexOfObject:self.day] inComponent:2 animated:YES];
    
    if (index == self.minuteArr.count - 1) {
        self.hour = [NSString stringWithFormat:@"%d时", [self.timeArr[3] intValue] + 1];
        self.minute = self.minuteInterval == 1 ? [NSString stringWithFormat:@"%ld分", (long)[self.timeArr[4] integerValue]] : self.minuteArr[0];
        [self.pickerView selectRow:[self.hourArr indexOfObject:self.hour] inComponent:3 animated:YES];
        [self.pickerView selectRow:self.minuteInterval == 1 ? ([self.minuteArr indexOfObject:self.minute]) : (0) inComponent:4 animated:YES];
    } else {
        self.hour = [NSString stringWithFormat:@"%ld时", (long)[self.timeArr[3] integerValue]];
        self.minute = self.minuteInterval == 1 ? [NSString stringWithFormat:@"%ld分", (long)[self.timeArr[4] integerValue]] : self.minuteArr[index+1];
        [self.pickerView selectRow:[self.hourArr indexOfObject:self.hour] inComponent:3 animated:YES];
        [self.pickerView selectRow:self.minuteInterval == 1 ? ([self.minuteArr indexOfObject:self.minute]) : (index + 1) inComponent:4 animated:YES];
    }
...
//    NSLog(@"点击了保存");
    ...
    NSString *month = self.month.length == 3 ? [NSString stringWithFormat:@"%ld", (long)self.month.integerValue] : [NSString stringWithFormat:@"0%ld", (long)self.month.integerValue];
    NSString *day = self.day.length == 3 ? [NSString stringWithFormat:@"%ld", (long)self.day.integerValue] : [NSString stringWithFormat:@"0%ld", (long)self.day.integerValue];
    NSString *hour = self.hour.length == 3 ? [NSString stringWithFormat:@"%ld", (long)self.hour.integerValue] : [NSString stringWithFormat:@"0%ld", (long)self.hour.integerValue];
    NSString *minute = self.minute.length == 3 ? [NSString stringWithFormat:@"%ld", (long)self.minute.integerValue] : [NSString stringWithFormat:@"0%ld", (long)self.minute.integerValue];
    
    self.selectStr = [NSString stringWithFormat:@"%ld-%@-%@ %@:%@", (long)[self.year integerValue], month, day, hour, minute];
    
    int num = [self compareDate:self.selectStr withDate:self.date];
//    NSLog(@"%d",num);
    
    if (num >= 0) {
//        NSLog(@"时间选择有误，请重新选择");
        [self showMessage:[NSString stringWithFormat:@"您预约的时间不能晚于当前时间,请重新选择"] duration:5];
        return;
    }
...
-(void)showMessage:(NSString *)message duration:(NSTimeInterval)time {
    ...
    CGSize labelSize = [message boundingRectWithSize:CGSizeMake(320, 999)
                                             options:NSStringDrawingUsesLineFragmentOrigin
                                          attributes:attributes context:nil].size;
    
    label.frame = CGRectMake(10, 5, labelSize.width + 20, labelSize.height+10);
    label.text = message;
        ...
        case 1: { // 月
            ...
            if ([self.year integerValue] > [self.timeArr[0] integerValue]) {
                ...
                if (month_value.integerValue < [self.timeArr[component] integerValue]) {
                    if (self.isSlide) {
                        [pickerView selectRow:[self.dataArray[component] indexOfObject:[NSString stringWithFormat:@"%ld月", (long)[self.timeArr[component] integerValue]]] inComponent:component animated:YES];
                    } else {
                        self.month = month_value;
                    }
            ...
            if ([self.year integerValue] > [self.timeArr[0] integerValue]) {
                ...
                if ([self.month integerValue] > [self.timeArr[1] integerValue]) {
                    ...
                    if (day_value.integerValue < [self.timeArr[component] integerValue]) {
                        if (self.isSlide) {
                            [pickerView selectRow:[self.dataArray[component] indexOfObject:[NSString stringWithFormat:@"%ld日", (long)time_integerValue]] inComponent:component animated:YES];
                        } else {
                            self.day = day_value;
                        }
                        // 如果选择的日大于当前日，就赋值日
