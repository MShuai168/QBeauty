...
- (instancetype)init
{
    if (self = [super initWithFrame:[[UIScreen mainScreen] bounds]]) {
        ...
        _pickerView.datePicker.maximumDate = [NSDate new];
        _pickerView.datePicker.datePickerMode = UIDatePickerModeDate;
        [self setStartDate:@"2010"];
        _selectDate = self.defaultDate;
        [self pushDatePicker];
    }
    return self;
}
...
*/
- (void)didFinishSelectedDate:(DataTimeSelect)block
{
    _selectBlock = block;
}
...
- (void)pushDatePicker
{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        weakSelf.pickerView.frame = CGRectMake(0, kWinH - kPVH - 64, kWinW, kPVH);
...
-(void)setStartDate:(NSString *)dateStr{
    ...
    NSDate *selected = [NSDate new];
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"yyyy-MM-dd hh:mm:ss"];
    NSRange range = NSMakeRange(0, 4);
    NSString *currentOlderOneDateStr = [dateFormatter stringFromDate:selected];
    
    NSString *dateStrA = [currentOlderOneDateStr stringByReplacingCharactersInRange:range withString:dateStr];
    [dateFormatter setTimeZone:[NSTimeZone timeZoneForSecondsFromGMT:8]];
    [dateFormatter setDateFormat:@"yyyy-MM-dd hh:mm:ss"];
    NSDate *startDate = [dateFormatter dateFromString:dateStrA];
    [self.pickerView.datePicker setDate:startDate];
    self.defaultDate = startDate;
