...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _placeHolderColor = COLOR_GRAY_MEDIUM;
    }
    return self;
}
...
- (void) drawPlaceholderInRect:(CGRect)rect
{
    ...
    NSDictionary* attr = [NSDictionary dictionaryWithObjectsAndKeys:
                          ...
                          NormalFontWithSize(15.0), NSFontAttributeName,
...
{
//    (SYSTEM_VERSION_GREATER_THAN(@"11.0")
 if (([[[UIDevice currentDevice] systemVersion] floatValue] >= 11.0)) {
     ...
     return CGRectMake(0, (bounds.size.height-size.height)/2 , bounds.size.width, size.height);
 }else {
     CGSize size = [[NSString string] sizeWithAttributes:[NSDictionary dictionaryWithObjectsAndKeys:self.font, NSFontAttributeName, nil]];
        ...
        */
        ...
        if (keyboardSize.height >= 260) {
                [UIView animateWithDuration:animationDuration
                                    ...
                                    options:animationCurve
                                 animations: ^{
                                     self.superview.frame = CGRectMake(frame.origin.x, SCREEN_HEIGHT-keyboardSize.height-frame.size.height, frame.size.width, frame.size.height);
...
- (void) notifierKeyboardWillHide:(NSNotification*)notification {
    ...
    if ([NSStringFromClass(self.superview.class) isEqualToString:@"UITableViewCellContentView"]) {
        ...
        [UIView animateWithDuration:animationDuration
                            ...
                            options:animationCurve
                         animations: ^{
                             self.superview.frame = CGRectMake(frame.origin.x, (SCREEN_HEIGHT-frame.size.height)/2, frame.size.width, frame.size.height);
